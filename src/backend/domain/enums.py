"""Enumerations shared by the three layers (docs/00_SCOPE_AND_DEFINITION_OF_DONE.md, 02)."""
...
class L1Decision(str, Enum):
    ...
    no_person = "no_person"
    #: detector answered too long ago to trust — docs/01_PIPELINE.md requires fail-open.
    stale = "stale"
    ...
    def permits_skip(self) -> bool:
        ...
        docs/00_SCOPE_AND_DEFINITION_OF_DONE.md: "L1 unavailable/stale 時，系統 fail-open，不把該狀態當成空房."
        """
        return self is L1Decision.no_person


class L2Outcome(str, Enum):
    """Why L2 ran, or why it did not (docs/02_DATA_AND_POLICY.md ``pipeline_runs``)."""
...
class L3Outcome(str, Enum):
    """Why L3 ran, or why it did not (docs/02_DATA_AND_POLICY.md ``pipeline_runs``)."""

    not_required = "not_required"
    called = "called"
    #: docs/01_PIPELINE.md: text-only is a *degraded* mode, allowed only when the clip is
...
class EscalationTrigger(str, Enum):
    """Who asked for L3 (docs/01_PIPELINE.md §L3)."""
...
class EventStatus(str, Enum):
    """Union of both state machines (docs/01_PIPELINE.md §Event state machine)."""
    ...
    def is_high_risk(self) -> bool:
        """High-risk states bypass L1 entirely (docs/00_SCOPE_AND_DEFINITION_OF_DONE.md item 8)."""
