"""Zero-dependency structured-output contracts (docs/01_PIPELINE.md §L2).

The backend deliberately avoids a third-party validation library so a
reviewer can clone the repo and run ``bun start`` / the test suite on a
stock CPython 3.11+ install (docs/04_SETUP_DEPLOY_VERIFY.md: Windows + WSL must work without a
...
``SchemaError`` carries a machine-readable ``code`` so the L2 repair path
            (docs/01_PIPELINE.md: "schema invalid 可 repair 1 次") can tell a malformed
...
class Schema:
    ...
    #: reject keys the contract does not declare (docs/02_DATA_AND_POLICY.md audit needs exact shapes)
