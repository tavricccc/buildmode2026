-- Care Agent — initial schema (docs/02_DATA_AND_POLICY.md §Data, Event, Policy 與稽核)
--
...
--
-- pipeline_runs — one row per vision window, carrying the
-- whole L1 -> L2 -> L3 -> policy path required by docs/00_SCOPE_AND_DEFINITION_OF_DONE.md item 10.
...
CREATE TABLE IF NOT EXISTS model_calls (
  ...
  -- 'invalid' is a first-class outcome, not an error: docs/01_PIPELINE.md requires an
...
CREATE TABLE IF NOT EXISTS events (
  ...
  -- the same replayed footage must not create a second event (docs/00_SCOPE_AND_DEFINITION_OF_DONE.md item 11)
...
CREATE TABLE IF NOT EXISTS transcripts (
  ...
  -- retention TTL (docs/01_PIPELINE.md §Audio); the sweeper deletes past this instant
