"""Google Gemini native REST client (docs/01_PIPELINE.md §L2, src/README.md).

This client explicitly stops pretending Gemini is OpenAI-compatible. It is called
in its own shape:
...
class GeminiClient:
    ...
    def upload_file(self, path: str | Path, mime_type: str, display_name: str = "clip") -> UploadedFile:
        """Resumable upload for media above the inline limit (docs/01_PIPELINE.md)."""
    ...
    def media_part(self, path: str | Path, mime_type: str, cleanup: list[UploadedFile] | None = None) -> dict[str, Any]:
        """Build the right part for this file's size (docs/01_PIPELINE.md §L2)."""
        path = Path(path)
