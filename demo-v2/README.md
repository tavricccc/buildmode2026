# Care Agent 舞台 Demo v2：全時 QwenVL Loop

交付時間：2026-09-06 10:00（Asia/Taipei）

本目錄定義舞台縮小版。完整書審架構位於 `../docs-implementation-v3/`。簡化版沿用完整架構的 Event、SQLite、REST、WebSocket 與 Agent contracts，不建立第二套不相容程式。

本版是目前最新可執行 Demo 基線：攝影機只作指定場域的有限感知來源；QwenVL loop 先產生 observation，state machine 再確認事件，Caregiver-facing 的摘要與通知只使用已聚合資料。~~冰箱主動詢問、食材記憶與保存期限提醒~~仍保留在舊產品方向文件中，未列入本版必做功能。
...
## 展示鏈
    ...
    預錄跌倒／喝水影片，以即時速度播放
      → 固定節拍 Continuous Vision Loop
      → 現場 Qwen3-VL-8B 4-bit 推論（無 Frigate／motion 篩選）
      → 跌倒／喝水 state machine
      → SQLite
      → WebSocket Dashboard
...
- Replay 播放期間，loop 依固定節拍持續分析最新窗口；背景／none 結果也要顯示。
- 跌倒與喝水各一支正例影片。
...
- 不啟動 RTSP camera；ReplaySource 直接產生與 live source 相同的 `FramePacket` contract。
...
- 完整 v3：13 個邏輯元件。
- Demo v2 啟用：12 個邏輯元件。Source Manager 仍由 ReplaySource 提供影格，Setup Service 仍負責模型準備；只停用 Audio Manager。Continuous Vision Loop 必須啟用。
- 舞台常駐程序：backend、Qwen runtime、frontend，共 3 個；Qwen 若內嵌 backend 可縮成 2 個。
