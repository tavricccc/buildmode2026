...
export function SourcePage({ status }: { status: Status | null }) {
  ...
  const statusLabel = browserState === "streaming" ? "串流中" : browserState === "requesting" ? "等待權限" : browserState === "connecting" ? "連線中" : browserState === "error" ? "串流錯誤" : active ? "分析中" : "未啟動";
  const statusTone = browserState === "error" ? "bad" : active ? "ok" : "muted";
      ...
      <div className="source-layout">
        <Card className="preview-card">
          <div className="preview-frame">
            {mode === "browser_camera" && browserActive ? <video ref={videoRef} autoPlay muted playsInline aria-label="瀏覽器攝影機即時畫面" /> : mode === "browser_upload" && browserActive ? <div className="preview-empty"><VideoCamera size={38} /><b>影片上傳中</b><span>{uploadProgress}% · 後端正在解碼並分析</span></div> : running ? <img src={snapshot} alt="最新分析影格" /> : <div className="preview-empty"><VideoCamera size={38} /><b>尚未接收影像</b><span>選擇來源後顯示即時或分析影像</span></div>}
            <div className="preview-label"><i className={`status-dot ${active ? "ok" : "muted"}`} />{mode === "browser_camera" && browserActive ? "瀏覽器即時畫面" : mode === "browser_upload" && browserActive ? "上傳影片處理中" : "分析用低頻預覽"}</div>
          </div>
