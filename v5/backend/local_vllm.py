"""OpenAI-compatible local vLLM transport shared by every v5 agent.

The original Longcare runtime uses one local multimodal model for observation
and the Main Agent.  v5 keeps its provider boundary, but this transport lets
...
class LocalVllmResponse:
    ...
    def candidate_tokens(self) -> int | None:
        """Gemini-compatible alias used by the shared v5 L2 service."""
...
class LocalVllmClient:
    ...
    def video_parts(self, frames: list[bytes], clip_url: str | None = None) -> list[dict[str, Any]]:
        # The verified v5 MiniMax path deliberately sends sampled image
