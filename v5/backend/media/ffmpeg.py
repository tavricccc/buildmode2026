"""FFmpeg helpers: probing, MJPEG decode, clip encode (v5 01, 04).
...
def decode_command(uri: str, fps: float, width: int, transport: str = "tcp") -> list[str]:
...
def encode_clip(
    ...
    v5 01 hands L2 and L3 a *short video*, not a frame array: a clip is
...
def decode_gray(jpeg: bytes, width: int, height: int, timeout: float = 5.0) -> bytes | None:
    ...
    OpenCV — which matters because v5 04 forbids a heavy install before
