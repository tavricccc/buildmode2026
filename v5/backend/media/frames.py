"""Frame packets and the bounded ring buffer (v5 01 §Media ingest).

v5 keeps only a short recent window — this is a care pipeline, not an
...
class FramePacket:
    ...
    height: int
    source_id: str
    #: ``rtsp`` or ``replay`` — both share this downstream contract (v5 01).
