...
v3 的可執行元件不拆成三個獨立 Agent service；三個產品概念以同一 backend 內的 logical-agent 邊界呈現：

| 產品概念 | v3 實作對照 | v3 首版狀態 |
|---|---|---|
| Context / World State Agent | Continuous Vision Loop、Local Vision Adapter、Event State Machine、Agent Orchestrator | 跌倒／喝水 state、source status、observation 與 uncertainty |
| Resident Interaction Agent | Audio Manager、Whisper transcript buffer、Speak／Reminder action contract | ~~冰箱主動詢問流程列入首版~~ 保留為後續產品場景；首版以 bounded speech pipeline 為主 |
| Caregiver Agent | Health Context、MiniMax analysis、Long-term Observer、Dashboard／Telegram L3 | 健康摘要、事件聚合、日／週 finding 與照護者通知 |

~~舊版「Frigate candidate event → 強模型」流程~~不再是 v3 入口；有限 camera source 仍只代表明確配置的單一場域，不代表全屋監控。完整 v3 的 source、SQLite、API、model 與驗證契約優先於舊 `docs/` 與 HTML。
