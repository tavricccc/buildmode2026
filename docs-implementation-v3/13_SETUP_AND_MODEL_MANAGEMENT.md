...
## 2. 首次啟動流程
    ...
    bun start
      → 檢查 Bun、Python、ffmpeg／video codec、磁碟與權限
...
1. Runtime：Python、video codec、可用 RAM、磁碟空間。
2. Vision model：預設 Qwen3-VL-8B-Instruct 4-bit；4B 為明示 fallback。
3. Speech model：選擇 Whisper model 與語言 Chinese。
4. Camera：RTSP credential、連線測試與 live preview。
5. Vision loop：interval、window seconds、max frames 與 benchmark 結果。
...
- POST /api/integrations/camera/test
- POST /api/integrations/vision-loop/benchmark
