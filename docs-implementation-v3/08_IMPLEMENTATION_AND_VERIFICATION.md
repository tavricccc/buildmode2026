...
- RTSP live source、bounded frame buffer、continuous loop、VAD／Whisper pipeline、transcript retention。
...
- 手機 RTSP → bounded frame buffer → 全時 QwenVL loop → backend observation／event。
...
| Live media | RTSP 斷線重連、固定節拍推論、pending 上限、frame stale 與 evidence 可取 |
...
- RTSP 未接好：舞台可使用 ReplaySource，但完整書審驗收仍視為未完成。
