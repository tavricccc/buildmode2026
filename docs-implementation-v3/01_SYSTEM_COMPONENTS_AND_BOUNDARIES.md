...
- 影片輸入：OpenCV 或 PyAV 直接連 RTSP；bounded ring buffer 保存最近窗口，不部署 NVR 或物件偵測器。
...
React Dashboard
   ↕ REST + WebSocket
FastAPI Application
   ├─ Source Manager ─ RtspSource / ReplaySource
...
| 元件 | 負責 | 不負責 | 主要輸出 |
|---|---|---|---|
| Source Manager | 統一 replay、RTSP 輸入、重連與生命週期 | 事件語意 | frame packet、source status |
| Audio Manager | 收音、VAD segmentation、ASR job 與 transcript buffer | 風險判斷、永久保存原始音訊 | speech segment、transcript |
| Continuous Vision Loop | 依固定節拍建立工作、只保留最新待處理窗口、記錄延遲與丟棄 | 事件判定、無界排隊 | vision job、loop metrics |
| Frame Buffer / Sampler | 持續保留短 ring buffer、依設定抽幀與縮放 | 判斷跌倒或喝水 | sampled frame、window ref |
...
- MVP 的 domain pipeline 使用單一 backend process 與 bounded async queues；Qwen、audio 與 frontend 可依 runtime 邊界另起 process，不拆成多個 domain microservices。
- RTSP decode、loop scheduler 與 domain agents 預設都在 backend；domain agents 不是額外服務。
...
- loop 週期不得短於實測 p95 推論時間；忙碌時覆蓋尚未開始的舊 job，只處理最新窗口，不能累積 backlog。
...
責任矩陣共 **13 個邏輯元件**：Source Manager、Audio Manager、Continuous Vision Loop、Frame Buffer / Sampler、Local Vision Adapter、Event State Machine、SQLite Repository、Agent Orchestrator、MiniMax Adapter、Policy Gateway、Realtime Broadcaster、Dashboard、Setup Service。

相較 v2，移除的 Frigate、go2rtc、MQTT 與 Apple detector 都是外部 runtime 依賴，並新增一個 Continuous Vision Loop 邏輯元件；因此不要用 Docker process 數去加總邏輯元件數。
