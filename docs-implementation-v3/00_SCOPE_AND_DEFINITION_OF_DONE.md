...
系統能從手機 RTSP live stream 與麥克風持續取得資料，由全時 QwenVL loop、Silero VAD 與 Whisper 建立多模態事件，把結果存入 SQLite、即時更新 Web Dashboard，並讓 MiniMax 使用健康快照與事件聚合摘要產生可追溯的健康／風險分析。Replay 是相同 source contract 的測試 adapter，不是完整交付的替代品。

核心展示鏈：

```text
手機 RTSP → bounded frame buffer → fixed-rate QwenVL loop
...
- 單一使用者、單一場域；不做多租戶。
- 手機 RTSP live stream 可由 backend 持續解碼、斷線重連與回報 source status。
- QwenVL loop 不依賴 person、motion 或其他前置事件；依設定節拍持續分析最新窗口。
...
- Dashboard 即時呈現 camera、vision loop、VAD、ASR、local VLM、MiniMax、事件、健康與 tools 狀態。
...
2. Live RTSP 可連續運作，全時 loop 能在無前置事件時仍產生 observation；Replay 使用相同 downstream contract。
