...
### `WS /ws/media?mode=demo_upload&event_start_ms=<unix milliseconds>`

瀏覽器影片上傳使用 WebSocket binary frames 分片傳送，之後送出 `{"type":"media.upload.complete"}`。`event_start_ms` 是影片第一幀在歷史上的實際時間，不是影片內 offset；後端會先保存在 `data/uploads/incoming/`，輸出 `*-480p.mp4`，再由 `ReplaySource(realtime=true)` 依影片內時長進入與瀏覽器攝影機相同的 `FrameWindow → L1 → L2 → L3 → Policy → SQLite/WebSocket` 流程。上傳期間會推送 `media.stream.ready`、`media.stream.ack`、`media.stream.progress`、`media.stream.processing`、`media.stream.completed` 或 `media.stream.failed`。
